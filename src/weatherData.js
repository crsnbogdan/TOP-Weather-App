async function getWeatherData(queryLocation) {
    let rawTodayWeatherData = await fetch(
        `https://api.openweathermap.org/data/2.5/weather?q=${queryLocation}&appid=29112d9d44d0009fdd0e9aa6b008cdee`, { type: "cors" }
    );
...
async function getForecastWeatherData(currentCityLat, currentCityLon) {
    let rawForecastWeatherData = await fetch(
        `https://api.openweathermap.org/data/2.5/onecall?lat=${currentCityLat}&lon=${currentCityLon}&exclude=hourly,minutely&appid=29112d9d44d0009fdd0e9aa6b008cdee`, { type: "cors" }
    );
